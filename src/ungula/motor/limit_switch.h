    ...
    class LimitSwitch {
        public:
            ...
            void configure(uint8_t pin, bool invertPolarity = false) {
                ...
                ungula::hal::gpio::configInput(pin_);
                ...
                const bool rawNow = ungula::hal::gpio::isHigh(pin_);
                stable_ = rawNow;
                lastRaw_ = rawNow;
            }
            ...
            ///   match core::time::tick_ms_t — no narrowing from the
            ...
            void update(int64_t nowMs) {
                ...
                bool rawState = ungula::hal::gpio::isHigh(pin_);
