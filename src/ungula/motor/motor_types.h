...
namespace ungula::motor
{
    ...
    enum class HomingPhase : uint8_t { MOVE_TO_HOME = 0, BACKOFF = 1, FINAL_APPROACH = 2, COMPLETE = 3 };
    ...
    enum class SpeedUnit : uint8_t { STEPS_PER_SEC = 0, MM_PER_SEC = 1, CM_PER_SEC = 2, DEGREES_PER_SEC = 3 };
    ...
    namespace stall
    {
        ...
        constexpr int32_t DEFAULT_SG_SCORE_LIMIT = 10; /// SG_RESULT score level that confirms a stall.
        constexpr float DEFAULT_STALL_FRACTION = 0.65F; /// SG drop to 65% of baseline = stall reading.
        constexpr float DEFAULT_SG_PER_SPS = 0.17F; /// SG_RESULT per step-per-second (motor dependent).
        constexpr uint16_t DEFAULT_SG_BASELINE_CAP = 300; /// Max baseline — SG_RESULT saturates at high speeds.
